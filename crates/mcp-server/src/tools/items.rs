...
///
/// Routes through the shared access-aware seam
/// [`ItemService::load_for_view_filtered`](trovato_kernel::content::ItemService::load_for_view_filtered)
/// (FR-8 Story 3.3, D-15): tap-integrated item-level `check_access` is enforced
/// and fields the caller may not see are dropped before serialization.
/// Unpublished items require per-type view permission or plugin-granted access
/// via `tap_item_access`; a denied or missing item is reported as "not found".
pub async fn get_item(
    ...
    // Route through the shared access-aware seam (FR-8 Story 3.3, D-15): item-level
    // `check_access` (tap-integrated) enforced, then restricted fields dropped
    // before serialization. `None` (missing or access-denied) ⇒ "not found" so
    // item existence is not revealed.
    let item = state
        .items()
