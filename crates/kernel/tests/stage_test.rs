...
//! Uses the shared [`TestApp`] and [`run_test`] infrastructure to avoid
//! exhausting virtual memory and prevent cross-runtime connection staling.
...
#[test]
fn stage_publish_moves_items_to_live() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn stage_has_changes_with_items() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn stage_has_changes_with_deletions() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn stage_publish_processes_deletions() {
    run_test(async {
        let app = shared_app().await;
...
async fn create_test_author(app: &common::TestApp) -> Uuid {
...
#[test]
fn conflict_detection_live_stage() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn conflict_detection_cross_stage_config() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn publish_with_resolution_cancel_on_conflict() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn publish_with_resolution_overwrite_all() {
    run_test(async {
        let app = shared_app().await;
...
#[test]
fn conflict_detection_live_modified_config() {
    run_test(async {
        let app = shared_app().await;
