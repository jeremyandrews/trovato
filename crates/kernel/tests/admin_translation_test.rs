...
fn the_translation_list_renders_each_language_and_its_state() {
    run_test(async {
        ...
        // The list links into the write path now that there is one: a language
        // with a translation offers to edit it, one without offers to add it.
        assert!(
            html.contains(&format!("/admin/content/{item}/translate/it")),
            "an existing translation links to its edit screen"
        );
        assert!(
            html.contains(">Add<"),
            "an untranslated language offers to add"
        );
    });
}
...
fn the_translation_page_renders_an_existing_and_a_missing_translation() {
    run_test(async {
        ...
        assert!(html.contains(&format!("Original {tag}")));
        // The stored title comes back as the value of the form's title input,
        // which is what makes it editable rather than merely displayed.
        assert!(html.contains(&format!("Tradotto {tag}")));
        assert!(
            html.contains("<form"),
            "the page carries the form that writes the translation"
        );
        assert!(
            html.contains(r#"name="_token""#),
            "and the form carries the same CSRF field the rest of the admin uses"
        );
        assert!(
            html.contains(r#"name="body""#),
            "the content type's own translatable field is on the form, got: {html}"
        );
