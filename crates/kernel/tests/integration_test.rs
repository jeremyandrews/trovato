...
/// TQ-F1 (test-quality gap) + RATE-1 enforcement: a burst of login attempts
/// from one client DETERMINISTICALLY trips the limiter. The login limit is
/// 5/min, so the first 5 attempts are processed (401 bad creds) and the 6th is
/// blocked with 429 — asserted exactly, not the tolerant "200-or-429" the audit
/// flagged. The client is identified by the trusted-proxy-forwarded IP
/// (`ClientIp`), so a fixed `X-Forwarded-For` maps to one bucket.
#[test]
fn e2e_login_rate_limit_blocks_after_threshold() {
    run_test(async {
        let app = shared_app().await;

        let unique = uuid::Uuid::now_v7().simple().to_string();
        let username = format!("rlburst_{}", &unique[..12]);
        // Unique bucket per test run (shared DB/Redis), reset to a clean slate.
        let fake_ip = format!(
            "10.77.{}.{}",
            unique.as_bytes()[0] % 255,
            (unique.as_bytes()[1] % 254) + 1
        );
        app.state.rate_limiter().reset("login", &fake_ip).await.ok();
        ...
        assert!(
            statuses[..5].iter().all(|s| *s == StatusCode::UNAUTHORIZED),
            "first 5 attempts should be processed (401 bad creds), got {statuses:?}"
        );
        assert_eq!(
            statuses[5],
            StatusCode::TOO_MANY_REQUESTS,
            "the 6th attempt must be rate-limited (429), got {statuses:?}"
        );
    });
}

/// TQ-F2 (test-quality gap) — session-fixation regression guard. Login must
/// rotate the session id (`session.cycle_id()`), so the pre-auth anonymous
/// session cookie must differ from the post-auth one. No test previously
/// asserted this; a refactor dropping `cycle_id` would silently pass CI.
#[test]
fn e2e_login_rotates_session_id() {
    run_test(async {
        let app = shared_app().await;

        let unique = uuid::Uuid::now_v7().simple().to_string();
        let username = format!("fixation_{}", &unique[..12]);
        let email = format!("{username}@test.com");
        app.create_test_user(&username, "testpass123", &email).await;
        let fake_ip = format!(
            "10.55.{}.{}",
        ...
        // Establish an anonymous session (the login page writes a CSRF token,
        // which persists the session and sets the `id` cookie).
        let (pre_cookies, _csrf) = fetch_csrf_token(app, "", "/user/login").await;
        let pre_id = session_id_value(&pre_cookies);
        assert!(
            pre_id.is_some(),
            "anonymous visit should establish a session; cookies: {pre_cookies:?}"
        );

        // Log in, reusing that anonymous session cookie.
        let resp = app
        ...
        let post_id = session_id_value(&post_cookies);
        assert!(
            post_id.is_some(),
            "login should set a session cookie; cookies: {post_cookies:?}"
        );
        assert_ne!(
            pre_id, post_id,
            "session id must rotate on login (session-fixation guard)"
        );
    });
}
...
//
// AC-W2/CSRF-2 (FR-6 audit, Critical): the batch API shipped fully
// unauthenticated (anonymous create/cancel/delete + IDOR read of any batch).
// The mutating routes now require authentication + a valid `X-CSRF-Token`, and
// every batch is scoped to its creator (`owner_id`) — non-owner, non-admin
// callers get 404 so batch ids cannot be probed. The happy-path tests below
// authenticate as admin; the `..._rejected` / `..._isolation` tests are the
// AC-W2 regressions.
// =============================================================================

/// Create an authenticated batch and return `(cookies, batch_id)`. Panics on
/// any non-201 to keep the call sites terse.
async fn create_batch_authed(
    app: &TestApp,
    cookies: &str,
    operation_type: &str,
) -> (String, String) {
    let (cookies, csrf) = fetch_csrf_token(app, cookies, "/").await;
    let response = app
        .request_with_cookies(
            Request::post("/api/batch")
                ...
                .body(Body::from(
                    json!({ "operation_type": operation_type, "params": {} }).to_string(),
                ))
                .unwrap(),
            &cookies,
        )
    ...
    assert_eq!(
        response.status(),
        StatusCode::CREATED,
        "authenticated batch create should succeed"
    );
    let body = response_json(response).await;
...
fn e2e_batch_create_operation() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_create", "password123", "batch_create@test.com")
            .await;
...
/// AC-W2 regression: an anonymous create must be rejected (401), not silently
/// accepted. This is the Critical that made the endpoint anonymous mutation.
#[test]
fn e2e_batch_create_anonymous_rejected() {
    run_test(async {
        let app = shared_app().await;
        ...
        let response = app
            .request(
                Request::post("/api/batch")
                    .header("content-type", "application/json")
                    .body(Body::from(
                        json!({ "operation_type": "anon_attempt", "params": {} }).to_string(),
                    ))
                    .unwrap(),
            )
            .await;
        ...
        assert_eq!(
            response.status(),
            StatusCode::UNAUTHORIZED,
            "anonymous batch create must be rejected"
        );
    });
}

/// AC-W2 regression: an authenticated create WITHOUT a CSRF token must be
/// rejected (403). Proves the CSRF half of the fix.
#[test]
fn e2e_batch_create_missing_csrf_rejected() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_nocsrf", "password123", "batch_nocsrf@test.com")
            .await;
        ...
        let response = app
            .request_with_cookies(
                Request::post("/api/batch")
                    .header("content-type", "application/json")
                    .body(Body::from(
                        json!({ "operation_type": "nocsrf", "params": {} }).to_string(),
                    ))
                    .unwrap(),
                &cookies,
            )
        ...
        assert_eq!(
            ...
            "authenticated batch create without CSRF must be rejected"
        );
    });
}
...
fn e2e_batch_get_operation() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_get", "password123", "batch_get@test.com")
            .await;

        let (cookies, batch_id) = create_batch_authed(app, &cookies, "test_get").await;

        // Get the operation status (owner read).
        let response = app
            .request_with_cookies(
...
/// AC-W2 regression (IDOR): a batch is visible only to its owner (or admin). A
/// second, unrelated authenticated user must get 404 — not the batch, and not a
/// 403 that would confirm the id exists.
#[test]
fn e2e_batch_ownership_isolation() {
    run_test(async {
        let app = shared_app().await;
        let owner_cookies = app
            .create_and_login_admin("batch_owner", "password123", "batch_owner@test.com")
            .await;
        // Owner is admin so create succeeds; the *other* user is a plain user
        // so admin-bypass does not mask the ownership check.
        let (owner_cookies, batch_id) = create_batch_authed(app, &owner_cookies, "owned_op").await;

        let other_cookies = app
            .create_and_login_user("batch_other", "password123", "batch_other@test.com")
            .await;

        // The other user cannot read the owner's batch.
        let response = app
            .request_with_cookies(
                Request::get(format!("/api/batch/{batch_id}"))
                    .body(Body::empty())
        ...
        assert_eq!(
            response.status(),
            StatusCode::NOT_FOUND,
            "a non-owner must not read another user's batch"
        );
        ...
        assert_eq!(
            response.status(),
            StatusCode::OK,
            "the owner must still read their own batch"
        );
    });
}
...
fn e2e_batch_get_nonexistent_returns_404() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_get404", "password123", "batch_get404@test.com")
            .await;
...
fn e2e_batch_cancel_operation() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_cancel", "password123", "batch_cancel@test.com")
            .await;

        let (cookies, batch_id) = create_batch_authed(app, &cookies, "test_cancel").await;

        // Cancel the operation (owner + fresh CSRF).
        let (cookies, csrf) = fetch_csrf_token(app, &cookies, "/").await;
        let response = app
            .request_with_cookies(
...
fn e2e_batch_delete_operation() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_delete", "password123", "batch_delete@test.com")
            .await;

        let (cookies, batch_id) = create_batch_authed(app, &cookies, "test_delete").await;

        // Delete the operation (owner + fresh CSRF).
        let (cookies, csrf) = fetch_csrf_token(app, &cookies, "/").await;
        let response = app
            .request_with_cookies(
                Request::builder()
...
fn e2e_batch_delete_nonexistent_returns_404() {
    run_test(async {
        let app = shared_app().await;
        let cookies = app
            .create_and_login_admin("batch_del404", "password123", "batch_del404@test.com")
            .await;
...
// =============================================================================
// Category / Tag REST mutation API auth (AC-W1 / CSRF-1, FR-6 audit, Critical)
//
// The `/api/category` and `/api/tag` write handlers (create/update/delete/
// set-parents) shipped fully unauthenticated with no CSRF — anonymous taxonomy
// destruction. They now require `administer categories` + a valid
// `X-CSRF-Token`. These are the AC-W1 regressions: the anonymous/no-CSRF paths
// must be rejected, and the authorized path must still work.
// =============================================================================

/// AC-W1 regression: an anonymous category create must be rejected (403), not
/// executed. The permission gate returns 403 for an unauthenticated JSON caller.
#[test]
fn e2e_api_category_create_anonymous_rejected() {
    run_test(async {
        let _lock = PLUGIN_STATE_LOCK.read().await;
        let app = shared_app().await;
        app.ensure_plugin_enabled("trovato_categories").await;
        ...
        let response = app
            .request(
                Request::post("/api/category")
                    .header("content-type", "application/json")
                    .body(Body::from(
                        json!({ "id": "anon_cat", "label": "Anon", "hierarchy": 0 }).to_string(),
                    ))
                    .unwrap(),
            )
            .await;
        ...
        assert_eq!(
            response.status(),
            StatusCode::FORBIDDEN,
            "anonymous category create must be rejected"
        );

        // And nothing was written.
        let exists: bool =
            sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM category WHERE id = 'anon_cat')")
                .fetch_one(&app.db)
                .await
                .unwrap();
        assert!(!exists, "anonymous create must not persist a category");
    });
}

/// AC-W1 regression: an authenticated category create WITHOUT a CSRF token must
/// be rejected (403).
#[test]
fn e2e_api_category_create_missing_csrf_rejected() {
    run_test(async {
        let _lock = PLUGIN_STATE_LOCK.read().await;
        let app = shared_app().await;
        app.ensure_plugin_enabled("trovato_categories").await;
        ...
        let cookies = app
            .create_and_login_admin("cat_nocsrf", "password123", "cat_nocsrf@test.com")
            .await;
        ...
        let response = app
            .request_with_cookies(
                Request::post("/api/category")
                    .header("content-type", "application/json")
                    .body(Body::from(
                        json!({ "id": "nocsrf_cat", "label": "NoCsrf", "hierarchy": 0 })
                            .to_string(),
                    ))
                    .unwrap(),
                &cookies,
        ...
        assert_eq!(
            response.status(),
            StatusCode::FORBIDDEN,
            "category create without CSRF must be rejected"
        );
    });
}

/// AC-W1 positive path: an admin with a valid CSRF token can create and then
/// delete a category through the REST API (the fix must not lock out the
/// legitimate caller).
#[test]
fn e2e_api_category_create_and_delete_authorized() {
    run_test(async {
        let _lock = PLUGIN_STATE_LOCK.read().await;
        let app = shared_app().await;
        app.ensure_plugin_enabled("trovato_categories").await;
        ...
        let cookies = app
            .create_and_login_admin("cat_authed", "password123", "cat_authed@test.com")
            .await;

        let unique = uuid::Uuid::now_v7().simple().to_string();
        let cat_id = format!("api_cat_{}", &unique[..12]);
        ...
        let response = app
            .request_with_cookies(
                Request::post("/api/category")
                    ...
                    .header("X-CSRF-Token", &csrf)
                    .body(Body::from(
                        json!({ "id": cat_id, "label": "API Category", "hierarchy": 0 })
                            .to_string(),
                    ))
                    .unwrap(),
                &cookies,
        ...
        assert_eq!(
            response.status(),
            StatusCode::CREATED,
            "authorized category create should succeed"
        );
        ...
        .await
        .unwrap();
        assert!(exists, "category should be created");
        ...
        let response = app
            .request_with_cookies(
                Request::builder()
                    .method("DELETE")
                    .uri(format!("/api/category/{cat_id}"))
                    .header("X-CSRF-Token", &csrf)
                    .body(Body::empty())
                    .unwrap(),
                &cookies,
        ...
        assert_eq!(
            ...
            "authorized category delete should succeed"
        );
    });
}
...
/// Extract the `id` session-cookie value from a `"k=v; k2=v2"` cookie string.
fn session_id_value(cookies: &str) -> Option<String> {
    cookies
        .split(';')
        .map(str::trim)
...
fn e2e_api_comment_crud() {
    run_test(async {
        ...
        let comment_role_id: uuid::Uuid = sqlx::query_scalar(
            "INSERT INTO roles (id, name) VALUES ($1, $2) \
             ON CONFLICT (name) DO UPDATE SET name = $2 RETURNING id",
        )
        .bind(uuid::Uuid::now_v7())
        .bind("comment_test_role")
        .fetch_one(&app.db)
        .await
...
/// G-COST-OPAQUE (p11j): the figure the kernel surfaces to a plugin on
/// `AiResponse.cost_estimate` is exactly `AiTokenBudgetService::estimate_cost` —
/// the same call the `ai-request` host makes on both the foreground and the
/// `ai_background` principal paths (they share the one post-provider code path).
/// A real provider response cannot be driven locally (the SSRF fence blocks a
/// loopback stub — G-SSRF-LOCAL), so cost propagation is verified at this seam:
/// a mock-priced `ai_pricing` config yields the priced cost, and an unpriced
/// model yields `None` (the honest "unknown", COALESCEd to $0.00 in the smoke
/// SUM).
#[test]
fn e2e_ai_cost_estimate_reflects_pricing_config() {
    run_test(async {
        let _lock = AI_BUDGET_LOCK.lock().await;
        let app = shared_app().await;
        let budget_svc = app.state.ai_budgets();
        ...
        budget_svc.save_pricing_config(&pricing).await.unwrap();

        // Priced model: 1000 prompt @ $0.005/1k + 500 completion @ $0.015/1k
        // = 0.005 + 0.0075 = 0.0125 — the exact figure surfaced on the response.
        let cost = budget_svc.estimate_cost("gpt-4o", 1000, 500).await;
        assert_eq!(
            cost,
            Some(0.0125),
            "the priced cost must be what estimate_cost returns"
        );

        // Unpriced model: None — no invented cost.
        let none = budget_svc
            .estimate_cost("some-unpriced-model", 1000, 500)
            .await;
...
/// P11c / D-42: a plugin's per-plugin **background** token budget is enforced
/// against its background usage (`user_id IS NULL` rows) and refuses further
/// calls once exhausted; per-user rows for the same plugin do NOT count toward
/// the plugin cap, and an unconfigured plugin stays unlimited.
#[test]
fn e2e_ai_plugin_background_budget_enforcement_deny() {
    run_test(async {
        let _lock = AI_BUDGET_LOCK.lock().await;
        let app = shared_app().await;
        ...
        let capped_plugin = "test_bg_plugin_capped";
        let free_plugin = "test_bg_plugin_free";
        ...
        // Per-plugin cap of 100 tokens for capped_plugin on this provider.
        let mut plugin_limits = HashMap::new();
        plugin_limits.insert(capped_plugin.to_string(), 100u64);
        let mut plugin_defaults = HashMap::new();
        plugin_defaults.insert(test_provider.to_string(), plugin_limits);
        ...
        // Background usage (user_id = None) that exceeds the plugin cap.
        budget_svc
            .record_usage(
                &app.db,
                UsageLogEntry {
                    user_id: None,
                    plugin_name: capped_plugin.to_string(),
                    provider_id: test_provider.to_string(),
                    operation: "chat".to_string(),
                    model: "gpt-4o".to_string(),
                    prompt_tokens: 80,
        ...
        // A per-USER row for the same plugin/provider must NOT count toward the
        // per-plugin background cap (the user_id IS NULL separation).
        budget_svc
            .record_usage(
                &app.db,
                UsageLogEntry {
        ...
        // Exhausted: 110 background tokens >= 100 cap ⇒ denied.
        let denied = budget_svc
            .check_plugin_budget(&app.db, capped_plugin, test_provider)
            .await
            .unwrap();
        assert!(
            !denied.allowed,
            "plugin over its background cap must be denied"
        );
        assert_eq!(denied.action, BudgetAction::Deny);
        assert_eq!(
            denied.used, 110,
            "only the user_id IS NULL rows are metered"
        );
        assert_eq!(denied.limit, 100);
        assert_eq!(denied.remaining, Some(0));

        // A plugin with no configured cap stays unlimited.
        let free = budget_svc
            .check_plugin_budget(&app.db, free_plugin, test_provider)
            .await
            .unwrap();
...
/// P11c / D-43: AI timeout config is read from `site_config` on each call, so a
/// change takes effect without a rebuild. Resolution honors per-operation and
/// per-provider overrides and the 150s epoch clamp.
#[test]
fn e2e_ai_timeout_config_read_and_resolution() {
    run_test(async {
        let _lock = AI_BUDGET_LOCK.lock().await;
        let app = shared_app().await;
        ...
        use trovato_kernel::services::ai_provider::AiTimeoutConfig;

        // Unset: defaults (request ~10s, background ~90s).
        let cfg = ai.get_timeout_config().await.unwrap();
        assert_eq!(
            cfg.resolve_timeout("chat", "prov-x", false),
            Duration::from_secs(10)
        );
        assert_eq!(
            cfg.resolve_timeout("chat", "prov-x", true),
            Duration::from_secs(90)
        );

        // Save a new config and confirm the very next read reflects it.
        let mut per_operation_secs = HashMap::new();
        per_operation_secs.insert("chat".to_string(), 45u64);
        let mut per_provider_secs = HashMap::new();
        per_provider_secs.insert("prov-slow".to_string(), 200u64); // over-ceiling
        let saved = AiTimeoutConfig {
            request_default_secs: 15,
            background_default_secs: 120,
            per_operation_secs,
            per_provider_secs,
            breaker_window_secs: HashMap::new(),
        };
        ai.save_timeout_config(&saved).await.unwrap();

        let cfg2 = ai.get_timeout_config().await.unwrap();
        assert_eq!(cfg2.request_default_secs, 15, "config change is read live");
        assert_eq!(
            cfg2.resolve_timeout("chat", "prov-x", true),
            Duration::from_secs(45),
            "operation override beats the background default"
        );
        assert_eq!(
            cfg2.resolve_timeout("chat", "prov-slow", true),
...
/// P11c / D-44: cost accounting — the pricing table drives a per-row cost
/// estimate, an unpriced model degrades to no cost, and a per-plugin **currency**
/// budget is enforced against summed background cost.
#[test]
fn e2e_ai_cost_accounting_and_currency_budget() {
    run_test(async {
        let _lock = AI_BUDGET_LOCK.lock().await;
        let app = shared_app().await;
        let budget_svc = app.state.ai_budgets();
        ...
        use std::collections::HashMap;
        use trovato_kernel::services::ai_token_budget::{
            AiPricingConfig, BudgetAction, BudgetConfig, BudgetPeriod, ModelPrice, UsageLogEntry,
        };

        // Price gpt-4o; leave other models unpriced.
        ...
        budget_svc
            .save_pricing_config(&AiPricingConfig { models })
            .await
            .unwrap();

        // Cost math per row, via the service (reads the saved pricing).
        let priced = budget_svc.estimate_cost("gpt-4o", 1000, 500).await;
        assert_eq!(priced, Some(7.5), "1000 in @2.5 + 500 out @10 = 7.5");
        // Unpriced model ⇒ None (tokens-only, no invented cost).
        assert_eq!(
            budget_svc.estimate_cost("mystery-model", 1000, 1000).await,
            None
        );

        // Currency cap of $10 for this plugin/provider.
        let mut plugin_costs = HashMap::new();
        plugin_costs.insert(plugin.to_string(), 10.0f64);
        let mut plugin_cost_defaults = HashMap::new();
        plugin_cost_defaults.insert(test_provider.to_string(), plugin_costs);
        budget_svc
            .save_config(&BudgetConfig {
                period: BudgetPeriod::Daily,
                action_on_limit: BudgetAction::Deny,
                defaults: HashMap::new(),
                plugin_defaults: HashMap::new(),
        ...
        // Two background rows @ $7.5 = $15 > $10 cap. One unpriced row (None)
        ...
        let denied = budget_svc
            .check_plugin_cost_budget(&app.db, plugin, test_provider)
            .await
            .unwrap();
        assert!(
            !denied.allowed,
            "$15 spent over a $10 currency cap must deny"
        );
        assert_eq!(denied.limit, 10.0);
        assert!(
            (denied.used - 15.0).abs() < 1e-9,
            "unpriced rows contribute 0; used was {}",
            denied.used
        );

        // A plugin with no currency cap is unlimited.
        let free = budget_svc
            .check_plugin_cost_budget(&app.db, "other_plugin", test_provider)
            .await
            .unwrap();
        assert!(free.allowed);
