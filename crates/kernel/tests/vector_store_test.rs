...
fn store_embedding_graceful_when_unavailable() {
    run_test(async {
        ...
        if store.is_available().await {
            ...
            // The item.type column has an FK to item_type; seed the type first.
            // (Only reached when pgvector is available — otherwise the else
            // branch below runs and no item is inserted.)
            sqlx::query(
                "INSERT INTO item_type (type, label, plugin) \
                 VALUES ('test_vec', 'Vector Test Type', 'test') \
