...
pub struct BatchOperation {
    ...
    /// Owner of the operation: the authenticated user who created it.
    ///
    /// Batch state lives in Redis with no per-row ACL, so ownership is scoped
    /// here and checked by the route handlers (a non-owner, non-admin caller
    /// gets 404). `Uuid::nil()` for legacy rows created before this field
    /// existed (`serde` default); those are treated as admin-only.
    #[serde(default)]
    pub owner_id: Uuid,
