...
impl BatchService {
    ...
    /// Create a new batch operation owned by `owner_id` (the authenticated
    /// user who requested it). Ownership is enforced by the route handlers on
    /// read/cancel/delete so one user cannot touch another's batch.
    pub async fn create(&self, input: CreateBatch, owner_id: Uuid) -> Result<BatchOperation> {
        let id = Uuid::now_v7();
