...
pub fn register_crypto_functions(linker: &mut Linker<PluginState>) -> Result<()> {
        ...
        .func_wrap(
            ...
            |mut caller: wasmtime::Caller<'_, PluginState>,
             ...
             -> i32 {
                ...
                write_string_to_memory(&memory, &mut caller, out_ptr, out_max_len, &hex)
                    .unwrap_or(-1)
            },
        )
        .into_anyhow()?;

    // hmac-sha256(key_ptr, key_len, msg_ptr, msg_len, out_ptr, out_max_len) -> i32
    ...
    // random-bytes(len, out_ptr, out_max_len) -> i32
    ...
    // constant-time-eq(a_ptr, a_len, b_ptr, b_len) -> i32
