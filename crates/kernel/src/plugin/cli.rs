...
pub async fn cmd_plugin_list(pool: &PgPool, plugins_dirs: &[PathBuf]) -> Result<()> {
    let discovered = PluginRuntime::discover_plugins(plugins_dirs).await;
...
pub async fn cmd_plugin_install(pool: &PgPool, plugins_dirs: &[PathBuf], name: &str) -> Result<()> {
    let discovered = PluginRuntime::discover_plugins(plugins_dirs).await;

    let (info, dir) = discovered.get(name).with_context(|| {
        format!(
            "plugin '{}' not found in {}",
            name,
            describe_search_path(plugins_dirs)
        )
    })?;
    ...
    // Anchor on the directory this plugin was discovered in, walking up past
    // its own directory and the plugins directory that contains it, so an
    // app-supplied plugin looks for build output under its own workspace.
    let workspace_root = dir
        .parent()
        .and_then(|plugins_root| plugins_root.parent())
        .unwrap_or(dir);
...
pub async fn cmd_plugin_enable(pool: &PgPool, plugins_dirs: &[PathBuf], name: &str) -> Result<()> {
...
/// Render a plugin search path for an error message.
fn describe_search_path(dirs: &[PathBuf]) -> String {
    dirs.iter()
