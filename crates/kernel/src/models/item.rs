...
impl Item {
    ...
    pub async fn find_by_id(pool: &PgPool, id: Uuid) -> Result<Option<Self>> {
        let item = sqlx::query_as::<_, Item>(
            "SELECT id, current_revision_id, type as item_type, title, author_id, status, created, changed, promote, sticky, fields, stage_id FROM item WHERE id = $1"
        )
        .bind(id)
        .fetch_optional(pool)
        .await
    ...
    pub async fn list_by_type(pool: &PgPool, item_type: &str) -> Result<Vec<Self>> {
        let items = sqlx::query_as::<_, Item>(
            "SELECT id, current_revision_id, type as item_type, title, author_id, status, created, changed, promote, sticky, fields, stage_id FROM item WHERE type = $1 ORDER BY created DESC"
        )
        .bind(item_type)
        .fetch_all(pool)
        .await
    ...
    pub async fn list_by_author(pool: &PgPool, author_id: Uuid) -> Result<Vec<Self>> {
        let items = sqlx::query_as::<_, Item>(
            "SELECT id, current_revision_id, type as item_type, title, author_id, status, created, changed, promote, sticky, fields, stage_id FROM item WHERE author_id = $1 ORDER BY created DESC"
        )
        .bind(author_id)
        .fetch_all(pool)
        .await
    ...
    pub async fn list_published(pool: &PgPool, limit: i64, offset: i64) -> Result<Vec<Self>> {
        let items = sqlx::query_as::<_, Item>(
            "SELECT id, current_revision_id, type as item_type, title, author_id, status, created, changed, promote, sticky, fields, stage_id FROM item WHERE status = 1 AND stage_id = 'live' ORDER BY sticky DESC, created DESC LIMIT $1 OFFSET $2"
        )
        .bind(limit)
        .bind(offset)
        .fetch_all(pool)
