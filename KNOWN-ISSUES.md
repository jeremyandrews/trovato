...
<!-- version:begin -->
What is outstanding in 0.104.0.
<!-- version:end -->
This is the list that would otherwise be a surprise, so it is written down
...
early `api_version` such as `"0.2"` is accepted by any kernel whose major is
also `0`. Nothing was ever released against the pre-freeze API, so no such plugin exists
