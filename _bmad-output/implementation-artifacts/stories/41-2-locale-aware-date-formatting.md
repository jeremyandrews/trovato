...
1. `format_date` Tera filter accepts an optional `locale` parameter: `{{ date | format_date(locale="de") }}`
2. When `locale` is omitted, defaults to `active_language` from template context (not hardcoded "en")
...
6. When both `locale` and `format` are provided, `format` takes precedence (explicit format overrides locale default)
...
- [ ] Define locale-to-pattern lookup table as a `phf::Map` or `HashMap<&str, &str>` constant (AC: #3, #4)
  ...
  - [ ] ru: `"%-d %B %Y"`
  - [ ] pl: `"%-d %B %Y"`
- [ ] Modify `format_date` filter in `crates/kernel/src/theme/engine.rs` to accept optional `locale` param (AC: #1)
- [ ] Modify `format_date` filter to read `active_language` from Tera context as fallback when `locale` not provided (AC: #2)
...
2. Locale pattern from lookup table (using explicit `locale` param or `active_language` from context)
