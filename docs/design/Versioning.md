...
At 0.99.0 the plugin API is `(0, 99)` and every manifest declares
`api_version = "0.99"`. At 1.0.0 the API becomes `(1, 0)` and every manifest
...
was worth. A reader who knows a site runs Trovato 0.99.0 now knows exactly which
...
`0.99.0` gives `(0, 99)`, declared as `KERNEL_API_VERSION` in
...
and does not change through the 0.99 series.
...
that moves 0.99 to 0.100. Through the 0.99 series the freeze is therefore
policy, held by review, not by the tool. At 1.0.0 the tooling and the policy
agree again, and a break requires a MAJOR bump plus a written justification.
...
- **Release tags** (`v0.99.0`): multi-platform (amd64 + arm64), tagged with the
