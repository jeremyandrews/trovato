...
At 0.102.0 the plugin API is `(0, 102)` and every manifest declares
`api_version = "0.102"`. At 1.0.0 the API becomes `(1, 0)` and every manifest
...
was worth. A reader who knows a site runs Trovato 0.102.0 now knows exactly which
...
`0.102.0` gives `(0, 102)`, declared as `KERNEL_API_VERSION` in
...
With a kernel at API 0.102:

| Plugin API | Compatible? | Reason |
|------------|-------------|--------|
| 0.102 | Yes | Exact match |
...
that moves 0.102 to 0.103. Before 1.0 the freeze is therefore policy, held by
...
- **Release tags** (`v0.102.0`): multi-platform (amd64 + arm64), tagged with the
