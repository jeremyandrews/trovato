...
At 0.103.0 the plugin API is `(0, 103)` and every manifest declares
`api_version = "0.103"`. At 1.0.0 the API becomes `(1, 0)` and every manifest
...
was worth. A reader who knows a site runs Trovato 0.103.0 now knows exactly which
...
`0.103.0` gives `(0, 103)`, declared as `KERNEL_API_VERSION` in
...
With a kernel at API 0.103:

| Plugin API | Compatible? | Reason |
|------------|-------------|--------|
| 0.103 | Yes | Exact match |
| 0.42 | Yes | Same major, older minor: the kernel provides everything it asks for |
| 0.104 | No | Needs host functions this kernel may not export |
...
that moves 0.103 to 0.104. Before 1.0 the freeze is therefore policy, held by
...
- **Release tags** (`v0.103.0`): multi-platform (amd64 + arm64), tagged with the
