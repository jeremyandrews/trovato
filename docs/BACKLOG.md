...
| Argus | `plugins/argus/M1-FRICTION.md` through `M4-FRICTION.md` in [argus](https://github.com/jeremyandrews/argus), extracted from this tree at 0.104.0 | 0.99 |
...
| BL-02 (SITE-2) | Nothing could write a content translation: the kernel read and overlaid `item_translation` and no route, API, config import path, host function or shipped plugin inserted into it. Two of the three write paths now exist. `POST /admin/content/{id}/translate/{lang}` saves one from the admin form with the usual `_token` protection, and a delete route withdraws it; `item_translation.<uuid>.<lang>.yml` is a config entity, so a set can ship translations and a translated site exports and re-imports. The SDK path is **not** added and is a finding: no WIT item write can carry a language, so it would need a signature change. | `crates/kernel/src/content/item_service.rs` (`save_translation`, `delete_translation`); `crates/kernel/src/routes/admin_translation.rs`; `crates/kernel/src/config_storage/direct.rs` | fixed (#94), SDK path open | 1.0, **Ritrovo gate** for the SDK path | additive |
| BL-03 (SITE-3, G-VARIABLES-DOUBLE-NAMESPACE) | `variables-get` and `variables-set` prefix every key with `plugin.{name}.` and a missing key returns the caller's default without a log line. Neither the WIT nor the SDK mentions the namespace, so a plugin cannot read a site-wide variable and an operator cannot see which key a plugin variable lives under. | `crates/kernel/src/host/variables.rs:46-48,58,106-107`; `crates/wit/kernel.wit:42-46`; `crates/plugin-sdk/src/host.rs:720-729` | partly fixed: the host now logs at warn level on the first miss per key per process, naming both the key asked for and the namespaced key it resolved to, so an operator can see where a plugin variable lives and a silent default is no longer silent. The other half is **open**: a plugin still cannot tell an unset variable from a set one, because the WIT declares `get -> string` with no `option` or `result`, and the SDK reads every negative return as a host failure — a sentinel for "unset" would make an ordinary unset variable look like a failure to every plugin already compiled. That is a contract change, so it needs an opt-in addition in a later minor: a second function returning `option<string>`, leaving `get` as it is. | 1.0.x | the log half was additive; the unset signal is frozen, and so is `get` reading outside the namespace |
...
| BL-41 (G-ADMIN-UI-IS-ADMIN-ONLY, Argus M3) | Every admin handler gated on `require_admin`, which checks `users.is_admin`, while the JSON item routes check `create {type} content`. A role granted content permissions could use the API and not the screens. All 111 call sites across 17 route files now check a declared permission, and `/admin/content/add/{type}` checks the same per-type string `/item/add/{type}` does. `/admin/plugins` keeps `require_admin` by design. | `docs/admin-permissions.md`; `crates/kernel/src/routes/helpers.rs:101-131` | fixed (#92) | closed | n/a |
...
| `G-TRANSLATION-NO-WRITE-PATH` | BL-02 | The admin form and config import both write now; `item_translation` is an entity type in `ENTITY_TYPE_ORDER`, placed after `item` so a set ships both in one pass. What remains is the SDK: `save-item` takes opaque item JSON and the host behind it builds `UpdateItem`, which has no `language` field, and `CreateItem` with `language: None` hardcoded, so a plugin cannot set even an item's own language. A plugin can still reach the table through `db.insert` by declaring it in `db_tables`, which skips cache invalidation, search and revisions. |
...
| BL-90 (G-PERM-GRID-SAVE-REVOKES-PLUGIN-GRANTS) | Saving the permission grid revoked every plugin permission from every role: the handler built each role's desired set by filtering `KERNEL_PERMISSIONS` against the submitted checkboxes, and the save had replace semantics, so a permission the grid never rendered looked deliberately unchecked. The form now states which permissions it rendered and `set_permissions_within` confines removals to that set, so a permission the grid did not show keeps whatever it had. The checkbox key is an index rather than the name with spaces replaced by underscores, which was not reversible once plugin names brought underscores in. | `crates/kernel/src/models/role.rs` (`set_permissions_within`); `crates/kernel/src/routes/admin_user.rs`; `templates/admin/permissions.html` | fixed (#93) | closed | n/a |
...
| BL-98 (G-LOCALE-STRINGS-NEVER-LOADED) | UI strings for any language but the default are never loaded, and nothing imports a `.po` file. `trovato_locale` preloads the default language and there is no other `load_language` call; `LocaleService::import_translations` has no caller anywhere in the tree: no route, no CLI command, no config entity. The tutorial ships `docs/tutorial/config/locale/it.po` and nothing reads it. A site configured in two languages serves the second one entirely in the first one's strings. The plugin that is supposed to do this cannot: `trovato_locale` is 47 lines describing itself as ".po file import support", it implements `tap_menu` and `tap_perm` only, it declares no host interfaces at all, and its two menu entries carry a `callback` with no `tap_api` behind it, so `/admin/config/locale` and `/admin/config/locale/import` are a startup warning (BL-13) and a 404. The same shape as `trovato_content_translation` in BL-02. | `crates/kernel/src/state.rs:645` (default only); `crates/kernel/src/services/locale.rs:74` (no caller); `plugins/trovato_locale/src/lib.rs:16-27`; `plugins/trovato_locale/trovato_locale.info.toml:8-11,25` | open | 1.0, **Ritrovo gate** | additive |
...
**BL-08, inline scripts under the kernel's own CSP.** Day one: account recovery,
session revocation, passkey management
...
- **Netgrasp `FRICTION.md`** claims verification at API `(1,0)`, which never
  ...
  meant to announce.
- **Argus `M4-FRICTION.md`** had no status note, and the first three of its
  remaining gaps were closed; both are corrected in `jeremyandrews/argus`, where
  Argus moved after 0.104.0, along with a fourth gap (BL-41) that closed at
  0.103.0. The M3 status note misses G-COMMENTS-UNRENDERED
