...
pub struct StubHostState {
    ...
    /// Per-`Store` resource limiter, mirroring the kernel's `PluginResourceLimiter`
    /// (WASM-4) so the invoke model can measure the limiter's real per-call cost.
    /// Default carries an empty name (no allocation); the invoke model stamps the
    /// plugin name and attaches it via `store.limiter(...)`, matching production.
    pub limiter: BenchResourceLimiter,
}

/// Per-`Store` resource limiter that mirrors the kernel's `PluginResourceLimiter`
/// (`crates/kernel/src/plugin/limits.rs`) byte-for-byte in shape and cost, so the
/// invoke benchmark measures the same per-call overhead production pays: the
/// count-cap reads plus a trap-on-breach growth check. Returns `Err` on breach
/// (a clean trap), exactly like the kernel limiter.
#[derive(Debug)]
pub struct BenchResourceLimiter {
    /// Maximum linear-memory bytes a single `Store` may grow to.
    pub max_memory_bytes: usize,
    /// Maximum table elements a single `Store` may grow to.
    pub max_table_elements: usize,
    /// Maximum instances / tables / memories per `Store`.
    pub max_instances: usize,
    pub max_tables: usize,
    pub max_memories: usize,
    /// Plugin name, carried for the trap message (as in production).
    pub plugin_name: String,
}

impl Default for BenchResourceLimiter {
    fn default() -> Self {
        Self {
...
impl wasmtime::ResourceLimiter for BenchResourceLimiter {
    fn memory_growing(
        &mut self,
        _current: usize,
        desired: usize,
        ...
        if desired > self.max_memory_bytes {
            return Err(wasmtime::Error::msg(format!(
                ...
                self.plugin_name, self.max_memory_bytes
            )));
        }
        Ok(true)
    }
    ...
    fn table_growing(
        ...
        if desired > self.max_table_elements {
            return Err(wasmtime::Error::msg(format!(
                ...
                self.plugin_name, self.max_table_elements
            )));
        }
        Ok(true)
    }
...
impl StubHostState {
    pub fn new() -> Self {
        Self {
            ...
            limiter: BenchResourceLimiter::default(),
        }
    }
...
pub fn create_linker(engine: &Engine) -> Result<Linker<StubHostState>> {
        ...
        .func_wrap(
            ...
            |mut caller: wasmtime::Caller<'_, StubHostState>,
             ...
             message_len: i32| {
                ...
                let plugin =
                    read_string(&memory, &caller, plugin_ptr, plugin_len).unwrap_or_default();
                let message =
    ...
    // =========================================================================
    // Benchmark host functions
    // =========================================================================

    // The guest imports `trovato:kernel/bench::noop` unconditionally (it backs
    // the OQ-11 `bench_noop_loop` export). Every linker that instantiates the
    // guest — the gate host here, the async host, and the OQ-11 host that builds
    // on this one — must define it, so it lives in the base linker.
    linker
        .func_wrap(
            "trovato:kernel/bench",
            "noop",
            |_caller: wasmtime::Caller<'_, StubHostState>| {},
        )
        .map_err(|e| anyhow::anyhow!("{e:#}"))?;
