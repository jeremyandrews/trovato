...
enum BenchmarkType {
    ...
    Oq11,           // OQ-11 boundary-cost baseline (host-call, serialize, alloc, dispatch gate)
    Invoke,         // OQ-11 invoke-path perf (FR-4a / Story 2.3) vs the 1ms budget
}
...
impl std::str::FromStr for BenchmarkType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            "oq11" | "oq-11" | "boundary" | "baseline" => Ok(BenchmarkType::Oq11),
            "invoke" | "invoke-perf" | "fr4a" | "fr-4a" => Ok(BenchmarkType::Invoke),
            _ => Err(format!(
                "Unknown benchmark: {s}. Use: all, handle, serialize, concurrency, async, payload, mutation, oq11, invoke"
            )),
        }
    }
}
...
fn print_help() {
    ...
    println!("                                      async, payload, mutation, oq11, invoke");
    ...
    println!();
    println!("    # OQ-11 boundary-cost baseline (host-call, serialize, alloc, 1ms dispatch gate)");
    println!("    cargo run --release -p trovato-phase0 -- --benchmark oq11");
    println!();
    println!("    # OQ-11 invoke-path perf (FR-4a / Story 2.3) vs the 1ms budget");
    println!("    cargo run --release -p trovato-phase0 -- --benchmark invoke");
    println!();
    println!("BUILDING GUEST WASM:");
    println!(
        "    cargo build --release --target wasm32-wasip1 --manifest-path benchmarks/phase0/guest/Cargo.toml"
    );
}

/// Path to the compiled guest plugin WASM (release preferred over debug).
///
/// The guest is a standalone package (see `guest/Cargo.toml`), so its build
/// output lands under `benchmarks/phase0/guest/target/`. Older builds (when the
/// guest shared the workspace target) landed under the repo-root `target/`; both
/// locations are checked for compatibility.
fn guest_wasm_path() -> &'static str {
    const CANDIDATES: [&str; 4] = [
        "benchmarks/phase0/guest/target/wasm32-wasip1/release/phase0_guest.wasm",
        "benchmarks/phase0/guest/target/wasm32-wasip1/debug/phase0_guest.wasm",
        "target/wasm32-wasip1/release/phase0_guest.wasm",
        "target/wasm32-wasip1/debug/phase0_guest.wasm",
    ];
    ...
    // Fall back to the canonical release path for the "not found" error message.
    CANDIDATES[0]
}
...
async fn main() -> Result<()> {
    ...
    let max_instances = std::cmp::max(config.concurrency * 2, 2000);
    ...
    if !wasm_path.exists() {
        ...
        println!(
            "  Build it with: cargo build --release --target wasm32-wasip1 --manifest-path benchmarks/phase0/guest/Cargo.toml"
        );
        return Ok(());
    }
    ...
    match config.benchmark {
        ...
        BenchmarkType::Oq11 => {
            // OQ-11 builds its own production-config engine(s) from the guest wasm
            // (it does not reuse the gate-benchmark host above).
            bench_oq11::run_oq11(&wasm_path).await?;
        }
        BenchmarkType::Invoke => {
            // OQ-11 invoke-path measurement (FR-4a / Story 2.3); builds its own
            // production-config engine from the guest wasm.
            bench_invoke::run_invoke_perf(&wasm_path).await?;
        }
    }
